...
class ExcelPicture(object):
    ...
    def process(self, output_file='_pic.xlsx', color_rounding=False, color_rounding_model=5.0):
        ...
        self.__sht.set_default_row(height=2)  # 行高单位是磅 换算比大概是11:1 使得单元格为1:1 正方形像素
        #        self.__sht.set_column('A:ZZZ', 50)
        self.__sht.set_column(0, 5000, width=0.2)  # 列宽单位是字符
        #        self.__sht.set_row(50)
        # 打开需要进行转换的图片
...
if __name__ == '__main__':
    path = os.getcwd()
    r = ExcelPicture(path+'/yq.jpg', ratio=1)
    r.process(path+'/yq.xlsx', color_rounding=True, color_rounding_model=5.0)